...
class Set_Cookie(object):
    ...
    def handle_js_content(self, resp):
        ...
        with open('excute_js_code.js', 'r', encoding='utf-8') as f:
            ...
            get_cgT = execjs.compile(excute_js_text).call("get_cgT")
            self.cookies['6HZbKHDjIEcgT'] = get_cgT
            self.session.cookies.update(self.cookies)
            print("生成 6HZbKHDjIEcgT 的值: ", get_cgT)
            print("生成 6HZbKHDjIEcgT 的长度: ", len(get_cgT))
    ...
    def set_first_cookie(self):
        ...
        try:
            self.cookies['6HZbKHDjIEcgS'] = first_response.cookies['6HZbKHDjIEcgS']
            self.session.cookies.update(self.cookies)
